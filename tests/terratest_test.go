...
func TestFull(t *testing.T) {
	...
	instanceName := fmt.Sprintf("test-policies-flow-ctrl-%s", uniqueId)
...
{
	"Name":        "{{ .name }}",
	"Description": "{{ .name }} Flow Control Policy.",
...
`
	...
	iassert.AssertMOComply(t, fmt.Sprintf("/api/v1/fabric/FlowControlPolicies/%s", moid), expectedJSONTemplate, vars)
}
